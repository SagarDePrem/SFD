...
%...Satellite data:
CD = 0.015*5668/(pi*2.3*2.3); %Drag coefficient %Assuming only the circular frontal area adds to drag
m = 5668; %Mass (kg)
A = pi/4*(1^2) ; %Frontal area (m^2)
%...Initial orbital parameters (given):
rp = RE + 243; %perigee radius (km)
ra = RE + 275; %apogee radius (km)
RA = 339.94*deg; %Right ascension of the node (radians)
i = 57*deg; %Inclination (radians)
...
% from t0 to tf:
t0 = 0; tf = 600*days; %Initial and final times (s)
...
options = odeset('reltol', 1.e-8,'abstol', 1.e-8, 'initialstep', T/10000,'events', @terminate);
global alt %Altitude
...
plot(apogee(:,1)/days, apogee(:,2)+RE,'b','linewidth',2)
hold on
plot(perigee(:,1)/days, perigee(:,2)+RE,'r','linewidth',2)
grid on
grid minor
xlabel('Time (days)')
ylabel('Altitude (km)')
