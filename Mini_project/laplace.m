% Estimate the ECI state vector <RV> of a satellite at time T(2) when its
% azimuth and elevation angles <AZI_ELE> are given for 3 different times
% <T> as viewed from the ground station <lat,lon,alt>. The function
% implements Laplace's method to perform the orbit fit. All angles are in
% radians and distances are in meters. Values in <T> must be Julian Dates.
function RV = laplace(lat, lon, alt, T, AZI_ELE)
    constants;
