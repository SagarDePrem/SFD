...
% ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
function [ascension,declination] = topoToEci(A,a,theta, phi, H)
% ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
%{
Function file rv_from_observe.m(modified heavily)
...
ascension=RA*360/2/pi;
declination=dec*360/2/pi;
%...Equations 5.57:;
%Rho = [cos(RA)*cos(dec) sin(RA)*cos(dec) sin(dec)]; 
%...Equation 5.63:
%r = R + rho*Rho;
%...Equation 5.84:
%decdot = (-Adot*cos(phi)*sin(A)*cos(a) + adot*(sin(phi)*cos(a) - cos(phi)*cos(A)*sin(a)))/cos(dec);
%...Equation 5.85
%RAdot = wE + (Adot*cos(A)*cos(a) - adot*sin(A)*sin(a) + decdot*sin(A)*cos(a)*tan(dec)) /(cos(phi)*sin(a) - sin(phi)*cos(A)*cos(a));
%...Equations 5.69 and 5.72:
%Rhodot = [-RAdot*sin(RA)*cos(dec) - decdot*cos(RA)*sin(dec),RAdot*cos(RA)*cos(dec) - decdot*sin(RA)*sin(dec),decdot*cos(dec)];
%...Equation 5.64:
